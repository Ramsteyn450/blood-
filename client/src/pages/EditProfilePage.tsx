...
import { CheckCircle, AlertCircle, Save, ArrowLeft, Zap } from 'lucide-react';
...
export default function EditProfilePage() {
  ...
  const [form, setForm] = useState({ name:'', phone:'', profession:'', weight:'', gender:'male', bloodType:'O+', takesTablets:'no', state:'', district:'', bio:'', publicNote:'', emergencyAvailable:false, visibleOnMap:true });
  ...
  useEffect(() => {
    if (user) setForm({ name:user.name||'', phone:user.phone||'', profession:user.profession||'', weight:String(user.weight||''), gender:user.gender||'male', bloodType:user.bloodType||'O+', takesTablets:user.takesTablets?'yes':'no', state:user.state||'', district:user.district||'', bio:user.bio||'', publicNote:user.publicNote||'', emergencyAvailable:user.emergencyAvailable||false, visibleOnMap:user.visibleOnMap!==false });
  }, [user]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const { data } = await api.put('/users/profile', { ...form, weight:parseFloat(form.weight), takesTablets:form.takesTablets==='yes' });
      updateUser(data.user); setSaved(true);
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-8 page-enter">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div><h1 className="font-display text-2xl font-bold text-gray-800">Edit Profile</h1><p className="text-gray-400 text-sm">Update your donor information</p></div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="card p-5">
          ...
          <div className="space-y-3">
            <div><label className="block text-sm font-semibold text-gray-600 mb-1.5">Full Name</label><input type="text" value={form.name} onChange={e => upd('name',e.target.value)} className="input-field" required /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-sm font-semibold text-gray-600 mb-1.5">Phone</label><input type="tel" value={form.phone} onChange={e => upd('phone',e.target.value)} className="input-field" /></div>
            ...
            <div><label className="block text-sm font-semibold text-gray-600 mb-1.5">Public Note</label><input type="text" value={form.publicNote} onChange={e => upd('publicNote',e.target.value)} placeholder="e.g. Happy to donate anytime!" className="input-field" maxLength={200} /></div>
            <div><label className="block text-sm font-semibold text-gray-600 mb-1.5">Bio</label><textarea value={form.bio} onChange={e => upd('bio',e.target.value)} className="input-field resize-none" rows={3} maxLength={300} /><p className="text-xs text-gray-400 mt-1">{form.bio.length}/300</p></div>
          </div>
        </div>
        ...
        <div className="card p-5">
          ...
          <div className="space-y-4">
            {[
              { key:'visibleOnMap', label:'Visible on Donor Map', desc:'Others can find and request you' },
